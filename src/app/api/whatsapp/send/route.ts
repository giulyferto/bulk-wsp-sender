...
import { shouldCancel, shouldSkipAndClear, clearSignals } from "@/lib/whatsapp/campaign-signals";
    ...
    async start(controller) {
      ...
      try {
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          if (shouldSkipAndClear(campaignId)) {
        ...
        if (!cancelled) {
          await campaignRef.update({ status: "done" });
